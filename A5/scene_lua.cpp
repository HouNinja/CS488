...
int gr_material_cmd(lua_State* L)
{
  ...
  double coef_reflection = luaL_checknumber(L, 4);
  double coef_refraction = luaL_checknumber(L, 5);
  double factor = luaL_checknumber(L, 6);

  data->material = new PhongMaterial(glm::vec3(kd[0], kd[1], kd[2]),
                                     glm::vec3(ks[0], ks[1], ks[2]),
                                     shininess, coef_reflection, coef_refraction, factor);
